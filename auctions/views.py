...
def detail(request, auction_id):
    ...
    highest_bid = Bid.objects.filter(auction=auction).order_by('-amount').order_by('-date').first()
    ...
    return render(request, 'detail.html', {'auction': auction, 'already_bid': already_bid,'highest_bid': highest_bid,'data':data})
    # try:
...
@login_required
def bidderslist(request,auction_id):
    auction = get_object_or_404(Auction, pk=auction_id)
    bidderslist=Bid.objects.filter(auction=auction).order_by('-amount').order_by('-date')
    return render(request,"bidderslist.html",{'bid':bidderslist})
